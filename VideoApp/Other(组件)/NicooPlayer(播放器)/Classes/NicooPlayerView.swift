...
//            self.hideLoadingHud()
    ...
    private func addUserActionBlock() {
        ...
        playControllViewEmbed.replayButtonClickBlock = { [weak self] (_) in
			self?.avItem?.seek(to: CMTime.zero, completionHandler: nil)
        ...
        playControllViewEmbed.pangeustureAction = { [weak self] (sender) in
            ...
            case .changed:
                ...
                case .PanDirectionHorizontal:
					...
					let positionValue = CMTimeMakeWithSeconds(Float64(durationValue) * Float64(draggedValue), preferredTimescale: (avItem.duration.timescale))
					avItem.seek(to: positionValue, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero, completionHandler: nil)
                    break
            ...
            case .ended:
                ...
                case .PanDirectionHorizontal:
					...
					avItem.seek(to: po, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero, completionHandler: nil)
